...
function getUserLessThanTwoSkill(users){
    let list = []
    for(let user of users){
        let {name, scores, skills, age} = user;
        if(skills.length < 2){
...
console.log(getUserLessThanTwoSkill(users));
...
for(const country of countries){
    let {name, capital, population, languages, ...rest} = country;
...
for(let student of students){
  ...
  let [HTM, CSS, JS, React] = skills;
  let  [a,b,c,d] = scores ;
  const list = [name, {HTM:a, CSS:b, JS:c, React:d}];
}
console.log( list );
...
function convertArrayToObject(students){
  ...
  for(let student of students){
    const [name,skills,scores] = student
     list.push({name:name,skills:skills,scores:scores})
  }
  return list; 
}
