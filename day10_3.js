...
// tao bang so luong ngon ngu duoc bn nuoc su dung
  const langUsedPopular = []
  for( let lang of Langes){
    const dem = counts.filter((l)=>l===lang)
    langUsedPopular.push([lang,dem.length])
  }

  // xap xep tu lon den be
  langUsedPopular.sort( (a,b) => b[1]-a[1] )
  
  let result= langUsedPopular.filter((value,index) => index<10 )
  console.log(result);
}
